...
schemaname,
tablename,
pg_size_pretty(total_size_bytes) AS table_size
FROM (
SELECT 
n.nspname AS schemaname,
c.relname AS tablename,
-- 如果是分区表，累加所有子分区大小；如果是普通表，直接取自身大小
COALESCE(
(SELECT sum(pg_total_relation_size(p.partitionschemaname || '.' || p.partitiontablename))
...
WHERE c.relkind = 'r' -- 仅查询普通表/分区表
AND n.nspname = 'public' -- 控制 Schema
-- 核心过滤：排除掉那些本身就是子分区的表（通常命名包含 _1_prt_）
AND NOT EXISTS (
SELECT 1 FROM pg_partitions p2 
WHERE p2.schemaname = n.nspname AND p2.partitiontablename = c.relname
)
) AS table_sizes
